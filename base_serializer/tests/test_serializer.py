...
class TestSerializer(TransactionCase):
    ...
    def test_serialize_loop(self):
        ...
        self.assertEqual(
            self.serializer.serialize(self.partner, False),
            [
                {
                    "name": self.partner.name,
    ...
    def test_inheritance_serialize(self):
        ...
        # Data comes from the base serializer
        self.assertFalse(serializer.field_ids)
        self.assertEqual(
            serializer.serialize(self.partner, False),
            [{"name": self.partner.name, "ref": self.partner.ref}],
        )
    ...
    def test_inheritance_deserialize(self):
        ...
        # Data comes from the base serializer
        data = [{"name": "other name", "ref": self.partner.ref}]
        self.assertFalse(serializer.field_ids)
        self.assertEqual(serializer.deserialize(data, False), data)
    ...
    def test_inheritance_import(self):
        ...
        data = [{"name": "other name", "ref": self.partner.ref}]
        self.assertFalse(serializer.field_ids)
        self.assertEqual(serializer.import_deserialized(data), self.partner)
        ...
        with self.assertRaises(ValidationError):
            second.import_deserialized(data)
        self.assertEqual(self.partner.name, "other name")
